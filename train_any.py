...
class Trainer:
    ...
    def train(self, model, weights_fn=None, save_fn=None, optim_weights=None, optimizer_state_dict=None): 
        ...
        optimizer, scheduler = self._build_optimizer_and_scheduler(self.config.train_cfg.optimizer, optim_weights, optimizer_state_dict, self.train_cfg)

        # initialize constants:
        # compute epochs
        if self.config.resume:
            epochs = self.config.epochs - int(self.config.resume_step/len(self._train_loader))
    ...
    def _build_optimizer_and_scheduler(self, optimizer, optim_weights, optimizer_state_dict, cfg):
        ...
        print(f"Creating {optimizer}, with lr {optimizer.param_groups[0]['lr']}")
...
class Workspace(object):
    ...
    def __init__(self, cfg):
        ...
        if resume:
            self._rpath = join(cfg.save_path, cfg.resume_path, f"model_save-{cfg.resume_step}.pt") 
            ...
            optimizer_state_dict = join(cfg.save_path, cfg.resume_path, f"model_save-optim-{cfg.resume_step}.pt")
            self.optimizer_state_dict = torch.load(optimizer_state_dict, map_location=torch.device('cpu'))
        else:
    ...
    def run(self):
        self.trainer.train(model=self.action_model, optimizer_state_dict=self.optimizer_state_dict)
        print("Done training")
