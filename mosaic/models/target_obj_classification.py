...
class TargetObjClassificator(nn.Module):
    ...
    def __init__(
        ...
        target_pos_in_dim = int(2*latent_dim) # agent embedding + demo embegging
        if action_cfg.n_layers == 1:
            self._action_module = nn.Sequential(nn.Linear(target_pos_in_dim, action_cfg.out_dim), nn.ReLU())
        elif action_cfg.n_layers == 2:
            self._target_object_classification = nn.Sequential(
                nn.Linear(target_pos_in_dim, action_cfg.hidden_dim), nn.ReLU(),
                nn.Linear(action_cfg.hidden_dim, action_cfg.out_dim), nn.ReLU(),
                nn.Linear(action_cfg.out_dim, 4)
                )
        else:
            raise NotImplementedError
